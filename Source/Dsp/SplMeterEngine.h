/*
  ------------------------------------------------------------------------------
    ...
    engaged, emits a test stimulus (sinusoid and/or white noise) on the
    selected outputs. The stimulus optionally passes through each output's
    trim + FIR correction, following the "Measure through correction" setting.
...
public:
    ...
    void setOutputsMask (juce::uint32 m)   { outputsMask.store (m); }
    void setThroughCorrection (bool b)     { throughCorrection.store (b); }
    ...
    /** Audio thread. Updates the mic RMS and, when a generator is engaged,
        renders the test signal on the selected outputs (optionally through
        their FIR). Returns true when it owns the audio: the monitoring matrix
...
private:
    ...
    std::atomic<juce::uint32> outputsMask { 0 };
    std::atomic<bool>  throughCorrection { false };
