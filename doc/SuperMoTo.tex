...
\begin{itemize}[nosep]
  ...
  \item After every run, SuperMoTo (re)writes a \texttt{readme\_measurement.md}
        in the folder: which channels have been measured, which one is the
        subwoofer, how many positions exist so far, and a log entry per run
        (mode, stimulus, duration, level, mic input, files written). This is
        what Group analysis's \textbf{Load measurement folder\ldots} button
        reads (Section~\ref{sec:grouploadfolder}) — it is regenerated from the
        folder's actual contents each time, so it stays correct even if files
        are added or removed by hand.
...
folder's \texttt{readme\_measurement.md} to find which channels were
