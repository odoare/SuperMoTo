...
\begin{itemize}[nosep]
  \item route up to \textbf{32 inputs to 32 outputs} (8 by default) through a
        full routing matrix, with per-crosspoint gain, EQ, phase and delay;
...
matrix engine. Audio enters on the discrete input channels (8 by default, up to
32), is summed through the active matrix configuration(s), passes the per-output
trim and FIR correction, is time-aligned across outputs, and finally the master
section (Level / Mute / Dim / Mono) is applied before the discrete outputs. The
active matrix size is set with the Inputs/Outputs selectors; the plugin bus
supports 8, 16, 24 or 32 discrete channels.
...
\begin{tikzpicture}[
  ...
  \node[block, right=of matrix] (frame) {Per-frame\\gain / phase};
...
column \emph{out} is a \textbf{frame} (crosspoint): a pure \emph{routing} cell
that controls how much of input \emph{in} is sent to output \emph{out}. All the
loudspeaker processing — trim, EQ, delay and the FIR correction — lives on the
\textbf{output}, since it describes the physical speaker; output settings are
global and therefore shared by all configurations.
...
\begin{tikzpicture}[
  ...
  \node[draw, circle, right=8mm of phase, fill=smtgold!40] (sum) {$\Sigma$};
  \node[block, right=8mm of sum] (trim) {trim};
  \node[block, right=5mm of trim] (eq) {4-band\\EQ};
...
\end{tikzpicture}
\caption{Routing frames (gain + phase) sum into each output, which then applies
the per-speaker trim, EQ, delay and FIR correction.}
\label{fig:frame}
\end{figure}
...
\begin{enumerate}[nosep]
  \item Set the plugin bus large enough (e.g.\ 24 in/out for order 3: 16
        B-format inputs, 16 speakers and a sub).
...
Produced formats: VST3, AU, Standalone (discrete in/out, selectable 8 / 16 /
24 / 32, default 8).
...
\begin{itemize}[nosep]
  \item A \textbf{configuration} (A--F) stores only the \emph{routing}: which
        inputs feed which outputs, with a gain, a phase flip and an active state
        per frame (Chapter~\ref{ch:matrix}).
  \item Everything that characterises a physical loudspeaker --- trim, EQ,
        time-alignment \textbf{delay} and \textbf{FIR} correction --- lives on
        the \textbf{output}, and is therefore \emph{shared by every
        configuration that uses that output} (Section~\ref{sec:outputchain}).
\end{itemize}
...
\begin{enumerate}[nosep]
  \item Raise the matrix size (bottom bar \textbf{Inputs}/\textbf{Outputs}) and
        the host bus to at least 8 in/out. Wire mains to 1--2, nearfields to
        3--4, sub to 5, headphones to 7--8.
...
\begin{enumerate}[nosep]
  \item Set the bus to at least 6 in/out and wire the channels as above.
...
\begin{enumerate}[nosep]
  \item Raise the matrix size and the host bus to at least 4 in / 9 out (16/16
        is convenient). Wire the eight monitors to outputs 1--8 and the sub to
        output~9; feed your AmbiX B-format onto inputs 1--4.
