...
\paragraph{How rare the failure is, and why that is the problem.} On that same campaign the defect occupied \emph{one bin}. Taking the largest phase step between neighbouring bins anywhere in the analysis band, the left loudspeaker reached \SI{14.1}{\degree} and the right \SI{154.9}{\degree}, the latter at \SI{697.8}{\hertz} and at exactly one of the \num{29632} bins in band. With~\eqref{eq:blendedaverage} in place both fall to \SI{6.6}{\degree}, and no bin anywhere exceeds \SI{90}{\degree}. A defect that occupies one bin in thirty thousand, leaves the magnitude untouched and is plainly audible is exactly the kind that a curve on a screen will not show and a listening test will.
