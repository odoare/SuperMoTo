  ...
  \item \textbf{Ambisonics}, where the count depends on which side of the
        ...
        \begin{itemize}[nosep]
          ...
          \item \textbf{Another decoder decodes.} If an upstream plugin
                ...
                the matrix as plain routing. SuperMoTo then contributes the
...
\begin{itemize}[nosep]
  ...
  \item a \textbf{delay}, \SI{0}{\milli\second} to \SI{100}{\milli\second}, fractional (sub-sample), for \textbf{time-aligning} the speaker;
...
The matrix settings themselves are deliberately not exposed as host parameters: $6 \times 32 \times 32$ frames with $\sim$10 fields each would swamp any host. They are saved with the plugin state. Only the master controls are real host parameters:
