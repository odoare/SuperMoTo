...
\paragraph{Stopping.} The button reads "Stop" while a run is going, and it cancels the run rather than merely ending it: the captures already written are deleted, and the status line says how many went. A run is all or nothing because half of one is worse than none --- a folder holding some channels of a position, with nothing in the manifest to say which, since that is only written when a run completes. Runs that finished are never touched, and neither is anything else in the folder: each run writes the next free position number per channel, so it can only ever delete its own files, and once they are gone the next run takes the numbers this one was using. A run that dies on a write error (a full disk, a folder that went away) cleans up the same way.
...
\begin{itemize}[nosep]
  \item the \textbf{channels} measured and which one is the subwoofer, plus the highest position number reached and the time of the last run. A channel that the preset names (Section~\ref{sec:outputchain}) carries that description with it, so the folder records which loudspeaker each capture came from. Names are kept from run to run: a run that cannot name a channel, a System run whose channels are inputs for instance, leaves the names an earlier run wrote;
