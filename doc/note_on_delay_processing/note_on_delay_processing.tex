...
the least-squares slope (\srcref{AnalysisEngine.cpp}{569--603}):
%
\begin{equation}
  \sigma \;=\;
  \frac{N \sum_n f_n \tilde\varphi_n - \bigl(\sum_n f_n\bigr)\bigl(\sum_n \tilde\varphi_n\bigr)}
       {N \sum_n f_n^2 - \bigl(\sum_n f_n\bigr)^2},
  \qquad
...
corrected main's phase is steered onto $\arg S'$.
...
\paragraph{The group shift.} A sufficiently negative $\Delta$ asks for the
subwoofer \emph{earlier} than the group reference, which no output delay can
express: $a_s$ would have to be negative. Rather than clamping (which would
silently break \eqref{eq:trim-offset}), the code pushes the whole group back by
the shortfall:
%
\begin{equation}
  \Sigma \;=\; \max\bigl(0,\; -(D - d_s + \Delta)\bigr).
\end{equation}
%
Every entry receives $\Sigma$ equally, so all relative timing (the only thing
that is audible) is untouched, at the cost of a little added latency. With
$\Sigma$ in place, $a_s \ge 0$ and $a_i \ge 0$ hold by construction, so the lower
clamp never fires.

\begin{remark}{Residual caveat}
The upper clamp at \SI{100}{\milli\second} can still fire, for a large positive
trim or a large shift on an already-spread group. If it does,
\eqref{eq:trim-offset} stops holding for the clamped entry and its crossover
alignment is designed for an offset it will not get. This is a corner the code
...
%
\begin{equation}
  \Delta\varphi \;=\; 2\pi\,|\Delta^\star|\,\beta\,(2 f_x)\,\ln 2,
  \qquad \beta = \max(\beta_{\mathrm{LF}}, \beta_{\mathrm{HF}}),
\end{equation}
%
and reports the estimate as usable only while $\Delta\varphi < \pi/2$, half the
angle at which the complex average collapses. Below that threshold the button is
disabled and the read-out says so, rather than offering a number that looks
authoritative. The trim itself stays available: the user can always set it by
ear.
...
shared controls rather than run per drag tick. The exported report records the
